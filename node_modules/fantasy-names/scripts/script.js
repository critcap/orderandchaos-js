...
// Generate names
function generate() {
	createCookie("selectedGenerator", nametype.value+"$"+getnamelist().value+"$"+quantity.value, 365);
	...
	if(typeof generator === 'function') {
		...
		for(var i = 0; i < quantity.value; i++) output.innerText += generator(Math.random() > 0.5 ? 1 : 0) + seperator;
	} else {
...
nametype.onchange = function() {
	...
	getnamelist().onchange = quantity.onchange = function() {generate();}
	generate();
}
